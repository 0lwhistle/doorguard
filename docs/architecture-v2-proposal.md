...
> 状态:**待评审**。本文是对"重构方向初稿"的修订,评审通过后:① 并入 `PROJECT_PLAN.md` §三
> (替换分层图与目录索引,保持单一事实源);② 按 §9 阶段启动目录迁移。
> 评审前**不迁移任何代码**,本文只定目标形态与迁移路径。
...
**跨服务调用(对既有"服务互不直调"的修订,待拍板 §10-1):**

- 写与命令:一律 event_bus 异步,请求/响应事件带 correlation id。
- 只读查询:允许直调目标服务暴露的**无阻塞 const 接口**,白名单仅两项——
  database 的特征快照、config 的配置读取。除此以外新加直调一律拒绝。
...
- **M1 纯机械移动,零行为变更**:git mv + CMake + include 路径一次到位;
  ...
  M1 期间禁止夹带任何功能改动。
- **M2 行为升级(每项独立提交、可单独回滚)**:① config 双文件 + 原子写 + 旧配置迁移;
  ② DB WAL + 单写者队列 + 特征缓存;③ registry 装配 + 看门狗 + 降级矩阵;④ ota 按需线程。
- **M3 补缺**:touch 模块化、as608/mfrc522 硬件接入时按新架构落位(不再产生新欠账)。
- 时机建议:B7 人脸联调收尾后的空窗期执行 M1(半天量级);M2 各项可择机穿插。

## 10. 待拍板事项

1. **跨服务只读直调放宽**(§1):database 特征快照、config 读取两处白名单——同意/否决。
2. **device_config 表冻结**(§4.1):新配置只进 JSON,存量迁移一次性做——同意/否决。
3. **RTSP 暂缓**:远程预览先走 WS 快照/MJPEG——同意/否决。
4. **迁移时机**:B7 联调后做 M1,还是立即——需要你定。
