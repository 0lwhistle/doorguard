...
| 当前进度 | **2026-09-21:人脸识别主线已切到自组 rknn 并上板跑通。**① **自组 rknn 视觉链路**(RetinaFace 检测 + ArcFace 识别;模型与实测见 `door-guard/models/README.md`):检测 320×320 i8 **6.7ms/帧**、识别 112×112 **56ms**、特征 **512 维 float32 = 2048B**(`DG_FEATURE_MAX` 已 512→2048,BLOB 免迁移);板上**检测黄框跟随 ✅**、**离线解码对拍 ✅**(分数 0.999、关键点解剖正确)、`vision_backend READY`;第三后端实现 `vision_rknn.c`,整体见 `door-guard/services/vision/README.md`。② **ROCKIVA 搁置为备选**:本版 SDK 快照 `rockiva_data_rk3576` 只发布前级检测一个文件(rk3588/rv1126 目录才有人脸件),需厂商渠道补模型;`vision_rockiva.c` 保留,缺模型时自行降级 ERROR 不影响其余业务。③ **NPU 库落位** `drv/npu/`(架构预留位):rknn 运行时薄封装 + RGA letterbox;全仓唯一 include `<rknn_api.h>` 的文件;附 `tools/npu_probe`(模型探针)与 `tools/rknn_det_test`、`rknn_rec_test`(离线对拍,不必等人站镜头前)。④ **UI 按用户反馈重做**:用户管理改为**编辑页模板**(添加/编辑同一页、姓名/权限/密码/人脸/指纹/IC 卡全字段、缺项显示“无”、全部多语言)、弹窗补取消、按钮去图标改文字、脸框 LOST 600ms 滞回防闪。⑤ **测试 28/28 绿**(新增 `test_rknn_face`/`test_npu_pre`/`test_enroll_flow`);交叉编译零告警。⑤b **2026-09-21 追加**:①修**连续命中循环**(1:N 持续上报 + FSM 无重复触发闸 → 反复开门/弹窗/日志把 UI 拖垮 = 主页面卡死与脸框闪烁的真凶;改为后端“一次在场只放行一次”);②**人脸质量体系**落位(`face_quality.c`:清晰度 Laplacian 方差/人脸像素/检测分,阈值走配置,已接入识别路径,19 项宿主测试);③**头像加密入库**(users 表加 avatar BLOB + 幂等迁移 + AES-256-CTR,独立存取接口,尺寸 160×160)。**UI 侧(拍摄页/头像显示)未做** → 见 `docs/tech/CAPTURE_AVATAR_HANDOFF.md`(唯一交接事实源)。⑤c **2026-09-21 再追加(拍摄录入 + 头像 UI 全量落地)**:①**同帧成对缓存**(vision_rknn `s_cap+s_snap` 一把锁:特征与 160×160 对齐大图同帧);②**拍摄时编码**(点击拍摄才编一次,`modules/jpeg/dg_jpeg` libjpeg 薄封装,先入 vision_service **照片槽**再 submit_feature,事件契约未动);③enroll 按同一 seq 取头像落库;④**拍摄页** `page_capture`(实时预览+质量实时提示 EV_VISION_QUALITY+不合格禁拍+拍/取消/重拍/回看,NAV_MAX_PAGES 9→10);⑤**头像显示** `ui/widgets/dg_avatar`(DB 解密→libjpeg 解码→lv_img_dsc_t;列表 40 缩略图/编辑页预览;清人脸/删用户随行消失,`db_user_clear_face` 连带清 avatar);⑥测试 30/30 绿(test_jpeg 新增、test_enroll_flow 扩头像链路),交叉编译零告警。⑤d **2026-09-21 三追加(主页状态栏 + 超时逻辑重做)**:主页左上时钟/右上网络图标(绿 WiFi/红 WiFi+红叉,net_info 只读轮询);**触摸按下沿钩子**(display→EV_UI_TOUCH,修复「只有待机页触摸算操作→菜单待久了回主页秒进待机」的根因);待机倒计时仅主页面计时+回主页重置;**菜单 15s 无操作自动回主页**(可配置 ui.menu_timeout_s,设备管理页与待机超时一起可调、即改即生效);门禁设置页待机超时迁往设备管理页;切页统一关弹窗。测试 30/30(FSM 新增 F10b)。⑤e **2026-09-22(用户六项反馈集中修,30/30 绿零告警)**:①**视觉推理移出主循环**——vision_rknn 新增 worker 线程(主循环回调只投单槽信箱,新帧顶旧帧立即归还),修复「有人脸时 UI 卡死/脸框时有时无/整机卡死卡帧」的总根因;②**FSM 在场判定闸**(window_done):1.5s 判定窗一次在场只判一次、FACE_LOST 撤销未决窗——路过不再误弹「验证失败」+垃圾日志;补上 access 对 EV_VISION_FACE_LOST 的缺失订阅;③**头像歪斜修复**:识别 ROI 改正方形整体平移夹取(消除 rw≠rh 各向异性挤压致对齐歪斜),新增对齐旋转角诊断日志;④**死机兜底**:主循环心跳监控线程(10s 无心跳退出交 S60 重拉)+ 预览三缓冲防撕裂 + 画布按 seq 去重;⑤**UI 层次感**(scrim 衬底 chip/半透明白描边/次要文字降透明度/菜单宫格卡片化/去冗余红叉);⑥**文档全面对齐**(default.json 死键清除、config/README 双文件模型、DEV_HANDBOOK、spec-database/network、models/README 等,详见 DEVLOG 2026-09-22 条)。⑥ **遗留**(见 DEVLOG 各条“下一步”):**拍摄录入/头像与主页状态栏、超时逻辑的板上人工验收**(代码已推板,真人对镜头逐条过)、质量阈值板上标定、1:N 阈值与查重阈值的余弦标定、指纹/IC 硬件接入、B8 活体算法、开机自动配网与 WiFi 驱动。⑤f **2026-09-22(深夜)video plane 直通预览落地**:NV12 经 dma-heap/RGA 旋转直送 VOP2
Overlay plane(zpos=0 underlay),预览 30fps 零 CPU(整机 CPU 降至单核 20%);fb 换 ARGB、
