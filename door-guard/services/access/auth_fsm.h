...
typedef struct {
    ...
    /** 本次在场已出过判定(命中或 1.5s 未命中):普通模式的匹配分支按它
     *  去重——1:N 每 300ms 持续上报一次,人站着时不得反复开门/反复弹失败。
     *  (2026-09-26 起"一次在场一次判定"由 FSM 独家收口;视觉后端不再设闸,
     *  否则站在镜头前点菜单的人永远等不到管理员模式的命中——普通模式早已
     *  放行过这一场。人离开(FACE_LOST)复位,下次到场重新判定 */
    bool window_done;

    /** 管理员模式:本在场已弹过「非管理员/陌生人」失败后,同一 user_id 的
     *  重复上报不再弹(1:N 每 300ms 重报,不去重会刷弹窗/日志);换人(如
     *  管理员上前)仍立即处理——spec §3「停留本模式继续尝试」。与 window_done
     *  分开:回普通模式的"不补判"置位 window_done,不得连管理员模式的命中
     *  一起挡掉。走开(FACE_LOST)或重新点菜单进管理员模式时复位 */
    bool admin_rejected;

    /** 画面里当前有没有脸(FACE_DETECTED 置位 / FACE_LOST 复位)。验证流程
