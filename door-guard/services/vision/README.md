...
camera NV12 1280×720(主循环 camera_poll 投递)
  ...
  └─ worker 线程:[每帧] **RGA 旋转到预览同向**(横置摄像头原始帧里人是
           躺着的,模型只认正立脸;2026-09-22 修,见下方「检测输入方向」)
...
**检测输入方向(2026-09-22 板上教训,别再改回去)**:摄像头横装,原始
NV12 帧里的人脸是 ±90° 横置的。曾直接把原始帧 letterbox 送检——离线对拍
(正立测试图)分数 0.999 全对,但板上横置脸的分数掉到 0.5~0.7、关键点/
框回归严重劣化(对齐旋转角应 ≈±90° 实测 4~8°、框近方形巨大、录出的头像
斜 45° 只剩半张脸),且空场景出现 0.5x 幻检把待机唤醒、刷「验证失败」。
修法:worker 先 `npu_pre_nv12_rotate` 把帧旋到与预览(DG_CAM_ROT=90)同向,
检测/ROI/对齐全在**预览=屏幕域**做,脸框坐标恒等发布,`rect_to_screen`
两段式映射删除。判定入口:「对齐旋转角」日志应 ≈0°(头部自然倾角)。
...
- **一次在场一次判定(2026-09-26 起归 FSM 收口)**:1:N 是持续上报的(每
  300ms 都会命中)。此前后端用 `s_granted_presence` 闸拦重复命中,但它会把
  **站在镜头前点菜单的人挡死**——普通模式早已放行过这一场,闸落了,管理员模式
  永远等不到命中(板上「管理员验证过不了」的根因)。现在后端持续上报,去重由
  access 的 FSM 按 `window_done`(普通模式)/ `admin_rejected`(管理员模式)
  在场内收口,`FACE_LOST` 复位;防"开门→结果→回普通→再开门"循环的职责不变。
...
| `face.det_threshold` | 检测器出框阈值(i8 模型空场景有 0.5x 幻检;正脸 0.85+) | 0.60 |
